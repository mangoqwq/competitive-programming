...
int main() {
  ...
  int N; cin >> N;
  Combinatorics<mint> combo(N * N);
  vector<vector<mint>> dp(N + 1, vector<mint>(N + 1));
  dp[1][1] = 1;
  for (int n = 2; n <= N; ++n) {
    for (int k = 2; k <= n; ++k) {
      for (int f = 1; f < n; ++f) {
        dp[n][k] += dp[n - f][k - 1] * dp[f][1] * combo.choose(n, f);
        // debug(n, k, f, dp[n - f][k - 1], dp[f][1], combo.choose(n, f));
      }
    }
    mint tot = accumulate(dp[n].begin() + 2, dp[n].end(), mint(0));
    dp[n][1] = mint(2).pow(n * (n - 1) / 2) - tot;
  }
  for (int k = 1; k <= N; ++k) {
    cout << dp[N][k].val() << '\n';
  }
