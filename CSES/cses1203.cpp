...
using mint = static_modint<998245223>;
...
int main() {
	...
	vector<mint> paths(N), rpaths(N);
	auto go = [&](vector<mint> &dp, int src, const vector<vector<int>> &adj) {
		vector<int> deg(N);
		for (int i = 0; i < N; ++i) {
	...
	vector<int> ans;
	for (int i = 0; i < N; ++i) {
		debug(paths[i].val(), rpaths[i].val());
		if (paths[i] * rpaths[i] == paths[N-1]) {
			ans.push_back(i);
		}
	}
	cout << ans.size() << '\n';
