...
struct Matrix {
  ...
  int N;
  Matrix(vector<vector<mint>> _a) : a{_a}, N(a.size()) {}
  Matrix operator*(const Matrix &b) const {
    vector<vector<mint>> ret(N, vector<mint>(N));
    for (int i = 0; i < N; ++i) for (int j = 0; j < N; ++j) {
      for (int k = 0; k < N; ++k) ret[i][j] += a[i][k] * b.a[k][j];
    }
    return ret;
  }
  vector<mint> operator*(const vector<mint> x) const {
    vector<mint> ret(N);
    for (int i = 0; i < N; ++i) for (int j = 0; j < N; ++j) {
      ret[i] += a[i][j] * x[j];
    }
...
int main() {
  ...
  while (T--) [&]() {
    ...
    map<int, mint> cache;
    auto calc = [&](int cnt) {
      if (cache.contains(cnt)) return cache[cnt];
      Matrix A({{1, 1}, {cnt, 0}});
      auto b = A.pow(N) * vector<mint>{1, 0};
      return cache[cnt] = b[0] + b[1];
    };
    auto factors = factor(K);
    map<int, int> mp;
    for (int x : factors) mp[x]++;
    debug(mp);
    for (auto [p, cnt] : mp) {
      ans *= calc(cnt);
    }
