...
/*
Let S_i be the set of sequences (N length, elements from 1..K) with no i
We can count B = S_1 U S_2 U ... U S_K using principle of inclusion/exclusion
But then B contains all the elements missing some i
So K**N - |B| is our answer
*/
...
int main() {
	...
	mint ans = 0;
	for (int i = 1; i <= K; ++i) {
		ans += mint(-1).pow(i+1) * c.choose(K, i) * mint(K-i).pow(N);
	}
