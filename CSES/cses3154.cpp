...
template<typename T>
int solveLinearMod(std::vector<std::vector<T>> &A, std::vector<T> &b, std::vector<T> &x) {
	int n = A.size(), m = x.size(), rank = 0, br = 0, bc = 0;
	if (n) assert((int)A[0].size() == m);
	std::vector<int> col(m); std::iota(col.begin(), col.end(), 0);
	...
	for (int i = 0; i < n; ++i) {
		...
		for (int j = 0; j < n; ++j) std::swap(A[j][i], A[j][bc]);
		T bv = 1 / A[i][i];
		for (int j = i + 1; j < n; ++j) {
			T fac = A[j][i] * bv;
			b[j] -= fac * b[i];
...
int main() {
	...
	std::vector<mint> x(M);
	auto rank = solveLinearMod(A, b, x);
	if (rank == -1) {
		std::cout << -1 << '\n';
	}
	else {
		for (int i = 0; i < M; ++i) {
			std::cout << x[i].val() << (i == M - 1 ? '\n' : ' ');
		}
	}
}
