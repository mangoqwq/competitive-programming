...
int main() {
	...
	vector<int> comp(uniq.begin(), uniq.end());
	for (int &x : vec) {
	...
	for (int i = 0; i < K; ++i) {
		cnt[vec[i]]++;
		st.set(vec[i], {cnt[vec[i]], vec[i]});
	}
	vector<ll> ans(N - K + 1);
	ans[0] = st.all_prod().arg;
	for (int i = 0; i < N - K; ++i) {
		cnt[vec[i + K]]++;
		st.set(vec[i + K], {cnt[vec[i + K]], vec[i + K]});
		cnt[vec[i]]--;
		st.set(vec[i], {cnt[vec[i]], vec[i]});
		ans[i + 1] = st.all_prod().arg;
	}
	for (int i = 0; i <= N - K; ++i) {
		cout << comp[ans[i]] << (i == N - K ? '\n' : ' ');
	}
}
