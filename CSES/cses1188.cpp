...
int main(){
    ...
    string s; cin >> s;
    int N = s.size();
    segtree<S, op, e> zero(N), one(N);
    function<void(int, int)> set = [&](int i, int v){
        if (v == 0) zero.set(i, 1), one.set(i, -1e9);
        if (v == 1) zero.set(i, -1e9), one.set(i, 1);
    };
    for (int i = 0; i < N; ++i){
        s[i] -= '0';
        set(i, s[i]);
    ...
    for (int i = 1; i <= M; ++i){
        ...
        s[x] ^= 1;
        set(x, s[x]);
        cout << max(zero.all_prod().ans, one.all_prod().ans) << " ";
    }
}
