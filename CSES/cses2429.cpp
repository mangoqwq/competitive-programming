...
int main() {
	...
	vector<vector<char>> row(N), col(N);
	int as = 0, bs = 0;
	...
	for (int k = 0; k <= N; ++k) {
		mint cur = 0;
		for (int aonb = 0; aonb <= k; ++aonb) {
			for (int bona = 0; aonb + bona <= k; ++bona) {
				int extra = k - aonb - bona;
				if (aonb > loneb) continue;
				if (bona > lonea) continue;
				if (extra > isor) continue;
				if (extra > isoc) continue;
				if (N - as - extra - aonb < 0) continue;
				if (N - bs - extra - bona < 0) continue;
				cur += c.choose(loneb, aonb) * c.choose(lonea, bona) * c.choose(isor, extra) * c.choose(isoc, extra) * c.fact(extra) * c.fact(N - as - extra - aonb) * c.fact(N - bs - extra - bona);
			}
		}
		int coef = (k % 2 == 0 ? 1 : -1);
		ans += coef * cur;
