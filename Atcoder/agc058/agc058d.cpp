...
int main(){
    ...
    int A, B, C; cin >> A >> B >> C;
    Combinatorics<mint> co(A+B+C);
    mint ans = co.fact(A+B+C) / co.fact(A) / co.fact(B) / co.fact(C);
    for (int i = 1; i <= min({A, B, C}); ++i){
        int a = A-i, b = B-i, c = C-i;
        mint ways = co.fact(a+b+c+i) / co.fact(a) / co.fact(b) / co.fact(c) / co.fact(i);
        mint x = co.fact(a+b+c+i-1) / co.fact(a) / co.fact(b) / co.fact(c) / co.fact(i-1);
        ans += mint(-1).pow(i) * (x*3*mint(2).pow(i-1) + (ways-x)*mint(2).pow(i));
    }
    cout << ans.val() << '\n';
}
