...
int main(){
	...
	// auto inv = [&](ll x){ return modpow(x, P-2, P); };
	auto normalize = [&](ll x){ return (x%P + P) % P; };
	...
	for (; d < (1 << 17); d *= 2){
		...
		for (int i = 0; i <= d; ++i){
			...
			if ((d-i) % 2 == 1) ptilde[i] = P - ptilde[i];
		}
		...
		for (int i = d+1; i <= 4*d+1; ++i){
			int k = i - (d+1);
			vals[i] = modmul(modmul(fact[i], inv_fact[k], P), ps[i], P);
		}
