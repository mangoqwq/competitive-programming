...
int main() {
  ...
  Combinatorics<mint> combo(5001);
  int T; cin >> T;
  while (T--) [&]() {
    int N; cin >> N;
    vector<int> a(N), nonblank(N+1), psa(N+1);
    for (int i = 0; i < N; ++i) {
      cin >> a[i];
      if (a[i] >= 0) nonblank[a[i]] = psa[a[i]] = 1;
    }
    for (int i = 1; i <= N; ++i) psa[i] += psa[i-1];
    int blank = ranges::count(a, -1);
    vector<vector<int>> d(N + 1, vector<int>(N + 1));
    for (int l = 0; l < N; ++l) {
      int cnt1 = 0;
      int reserve = 0, cnt2 = 0;
      vector<int> available(N);
      for (int r = l+1; r <= N; ++r) {
        if (a[r - 1] == -1) cnt1++, reserve++;
        else available[a[r-1]] = 1;
        while (true) {
          if (cnt2 < N && available[cnt2]) {
            cnt2++;
          }
          else if (reserve && !nonblank[cnt2]) {
            cnt2++; reserve--;
          }
          else break;
        }
        if (cnt2 < N) d[cnt1][cnt2 + 1]--;
        d[cnt1][0]++;
      }
    }
    mint ans = 0;
    for (int cnt1 = 0; cnt1 <= N; ++cnt1) {
      for (int cnt2 = 1; cnt2 <= N; ++cnt2) {
        d[cnt1][cnt2] += d[cnt1][cnt2-1];
        if (!d[cnt1][cnt2]) continue;
        int taken = cnt2 - psa[cnt2-1];
        ans += d[cnt1][cnt2] * combo.permute(cnt1, taken) * combo.fact(blank - taken);
      }
    }
    cout << ans.val() << '\n';
  }();
