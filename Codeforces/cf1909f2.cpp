	...
	while (T--) [&]() {
		...
		auto solve = [&](int a1) -> mint {
			...
			for (int i = 2; i <= N; ++i) {
				...
				int lo_rows = j;
				mint cur = 0;
				for (int lo = 0; lo <= min({lo_rows, dots, diff}); ++lo) {
					int hi = dots - lo;
					mint v1 = c.choose(lo_rows - a[j], lo) * c.permute(diff, lo);
					mint v2 = c.choose(i - a[j] - lo, hi) * c.permute(diff, hi);
					cur += v1 * v2;
				}
				ret *= cur;
				j = i;
		...
		mint ans = 0;
		if (a[1] == 0) ans = solve(0);
		else if (a[1] == 1) ans = solve(1);
		else if (a[1] == -1) ans = solve(0) + solve(1);
		cout << ans.val() << '\n';
	}();
}
