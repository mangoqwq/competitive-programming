...
int main(){
    ...
    int N, M; cin >> N >> M;
    mint::set_mod(M);
    Combinatorics<mint> c(3*N + 1);
    vector<mint> cnt(4);
    cnt[0] = 1;
    cnt[1] = 2 * (c.fact(2*N)) - c.fact(N);
    cnt[1] -= cnt[0];
    cnt[2] = 2 * c.choose(2*N, N) * c.fact(2*N) * c.fact(N);
    mint sub = 0;
    for (int i = 0; i <= N; ++i){
        mint cur = c.choose(2*N - i, N);
        cur *= c.choose(N, i) * c.choose(N, i);
        sub += cur;
    }
    sub *= c.fact(N) * c.fact(N) * c.fact(N);
    cnt[2] -= sub;
    cnt[2] -= cnt[1] + cnt[0];
    cnt[3] = c.fact(3*N) - cnt[2] - cnt[1] - cnt[0];
    mint ans = 0;
    for (int i = 0; i < 4; ++i){
        ans += i * cnt[i];
    }
