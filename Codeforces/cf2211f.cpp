...
int main() {
  ...
  int T; cin >> T;
  Combinatorics<mint> c(2e6);
  while (T--) [&]() {
    int N, M; cin >> N >> M;
    map<pair<int, int>, mint> cache;
    mint ans = 0;
    auto calc = [&](int l0, int l3) {
      return c.choose(l0 + l3 + M, M - 1);
    };
    auto go = [&](auto self, int l, int r, int l_eq, int r_eq, int dep) -> void {
      if (l > r) return;
      int mid = (l + r) >> 1;
      self(self, l, mid - 1, l_eq, mid, dep + 1);
      self(self, mid + 1, r, mid, r_eq, dep + 1);
      mint cur = 0;
      cur = calc(mid - 1, N - mid);
      if (l_eq != 0) cur -= calc(l_eq - 1, N - mid);
      if (r_eq != N+1) cur -= calc(mid - 1, N - r_eq);
      if (l_eq != 0 && r_eq != N+1) {
        cur += calc(l_eq - 1, N - r_eq);
      }
      ans += dep * cur;
    };
    go(go, 1, N, 0, N+1, 1);
