...
int main(){
    ...
    while (T--){
        ...
        atcoder::lazy_segtree<ll, op, id, ll, mapping, composition, e> st(K+2);
        st.set(K+1, cold[a[0]]);
        ...
        for (int i = 1; i < N; ++i){
            ...
            ll val = st.all_prod() + cold[a[i]];
            val = min(val, st.get(a[i]) + hot[a[i]]);
            st.apply(0, K+2, (a[i] == a[i-1] ? hot[a[i]] : cold[a[i]]));
            st.set(a[i-1], min(st.get(a[i-1]), val));
            // for (int j = 1; j <= K+1; ++j){
